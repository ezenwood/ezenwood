											...
											<c:choose>
												<c:when test="${fn:length(QNAListMap) > 0}">
													<c:forEach items="${QNAListMap}" var="row">
														<tr>
															<td><fmt:formatDate value="${row.QNA_DATE }"
																	type="both" dateStyle="medium" timeStyle="medium" /></td>
															<td class="title"><c:if
																	test='${row.QNA_SECREATE=="Y" }'>
																	<img alt="비밀글"
																		src="/ezenwood/resource/image/secret.png">
																</c:if> <c:choose>
																	<c:when
																		test='${row.QNA_SECREATE=="Y" && row.QNA_WRITER==MEMBER_NUM }'>
																		<a href="/ezenwood/goods/qna/${row.QNA_NUM }"
																			name="title">${row.QNA_TITLE }</a>
																	</c:when>
																	<c:when
																		test='${row.QNA_SECREATE=="Y" && row.QNA_WRITER!=MEMBER_NUM }'>
                                                               ${row.QNA_TITLE }
															...
															<td><c:choose>
																	<c:when test="${row.QNA_RE_GB =='Y'}">답변 완료</c:when>
							...
							<div class="btn_right_box">
								<c:if test="${MEMBER_ID !=null}">
									<button type="button" class="btn_write"
										onclick="insu(${GOODS_NUM})">문의하기</button>
								</c:if>
							</div>
						</div>
